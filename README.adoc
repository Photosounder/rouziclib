...
With link:rouziclib/interpreter/expression.c[expression.c] we can parse mathematical expressions such as `0.5 + 0.5cos(2pi (x-1))` into RLIP. My expression parser offers several advantages over https://github.com/codeplea/tinyexpr[TinyExpr] (which I would previously use extensively) which compel me to move away from the latter:

- My expression parser works with integers (which are used as results for comparisons) and either doubles or the custom arithmetic format whereas TinyExpr only uses doubles, integers can't even be used.
