...
Floating point approximations for the cosine at different levels of precision (up to 4.3e-32 with the double-double implementation), in turns or radians.
...
With link:rouziclib/interpreter/expression.c[expression.c] we can parse mathematical expressions such as `0.5 + 0.5cos(2pi (x-1))` into RLIP. My expression parser offers several advantages over https://github.com/codeplea/tinyexpr[TinyExpr] (which I previously would use extensively) which compel me to move away from the latter:

- My expression parser works with integers and either doubles or the custom arithmetic format whereas TinyExpr only uses doubles, integers can't even be used.
- Implied multiplications are supported. For instance `2x` means the same thing as `2*x` and `x cos(pi x)` the same as `x*cos(pi*x)`.
- TinyExpr parses recursively, generates a recursive data structure and the execution is also recursive, which isn't optimal for clarity and speed of execution. In contrast my parser proceeds in simple loops over a string and then a symbol array, produces a flat RLIP listing which is itself compiled into a flat list of opcodes.
