...
:imagesdir: tools/img
image::rouziclib_bytes.png[rouziclib_bytes.png,align="center"]
...
** link:rouziclib/general/portable_simd.h[portable_simd.h] +
Portable SIMD functions based on the WebAssembly SIMD feature set. The idea is that compilers can autovectorise these functions, otherwise it falls back to scalar loops.
...
Stringify macros, basic math macros and a macro to detect GCC.
...
Functions to compute intersections between lines, find the distance between a point and a line, find the segment of a line inside a rectangle, intersect rectangles, intersect a circle with a rectangle, find if a point is inside a rectangle or circle, or find the angular radius of a circle from a point.
...
Functions to find how to best fit squares or rectangles in an area, manage a Hilbert curve grid, fit a rectangle inside a rectangular area or subdivide a rectangle, scale coordinates between pixel or rectangle coordinates and the area rectangle it fits into.
...
** link:rouziclib/math/polynomial_signal.c[polynomial_signal.c] / link:rouziclib/math/polynomial_signal.h[polynomial_signal.h] +
Functions to turn a sampled signal (like a sampled sound) into a "polynomial signal" which is a series of polynomial chunks that represent the signal in a continuous manner according to sinc interpolation, and a function to quickly evaluate values at any point in the signal. The point is to have access to perfect sinc interpolation for individual points with none of the computational cost.
...
** link:rouziclib/graphics/srgb_simd.c[srgb_simd.c] / link:rouziclib/graphics/srgb_simd.h[srgb_simd.h] +
SIMD approximations that are faster than even look up tables. Approximations for log2, exp2, non-overflowing integer division by 2.4 by right shifts, sRGB conversions.
...
** link:rouziclib/libraries/clFFT-amalgamated.c[clFFT-amalgamated.c] / link:rouziclib/libraries/clFFT-amalgamated.h[clFFT-amalgamated.h] +
Amalgamation and conversion from C++ to C of clFFT.
...
// Inputs
static double x;
static rlip_inputs_t inputs[] = { RLIP_FUNC, {"x", &x, "pd"} };
...
[source,C]
----
static rlip_inputs_t inputs[] = { RLIP_REAL_DOUBLEDOUBLE, RLIP_FUNC, {"x", &x, "pd"} };
----
