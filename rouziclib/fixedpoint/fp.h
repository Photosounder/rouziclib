...
static INLINE int32_t log2_ffo64(uint64_t x)	// returns the number of bits up to the most significant set bit so that 2^return > x >= 2^(return-1)
{
	/*
	   Max iterations vs table size:
...
static INLINE int64_t fpdiv_d2(int32_t y, int32_t x, int32_t outfmt)	// ~24.3 cycles, max error (by division) 32.42e-9
{
...
static INLINE uint32_t isqrt_d1i(uint64_t x)
{
	...
	const uint32_t ish=64-lutsp, ip=32-prec, tp=32+abdp-prec, tbd=(ish-tp);	// the shift for the index, bit precision of the interpolation, the precision for t and how to shift from frx to t
...
static INLINE int32_t fplog2_d1i(uint32_t x)		// input is in 32.0, output is in s5.26
{
...
static INLINE uint32_t fpexp2_d1i(int32_t x)	// input is s5.26, output is in 32.0
{
...
static INLINE int32_t fpcos_d2(uint32_t x)		// 13 cycles
{
...
static INLINE int32_t fpcos_d1i(uint32_t x)		// does the equivalent of cos(2.*pi*x), x = [0.0 , 1.0[ = [0 , 2^32[ = 0.32 fp value
{
...
static INLINE int64_t fpatan2_d2(int32_t y, int32_t x)	// 62 cycles, average error 0.00274", max error 0.00893" (16 metres on the Moon, 27 cm on the surface of the Earth)
{
	...
	t = fpdiv(ya-xa, d, fmt);	// '/d' normalises distance to the unit diamond, immediate result of division is always <= +/-1^ds
...
static INLINE int32_t fpwsinc_d1i(int32_t x)	// Windowed sinc function in the [-2 , 2] range, input is 2+.24, output is s1.30
{
	#include "fpwsinc_d1i.h"	// includes the one-sided gaussian LUT as generated by tablegen.exe, the entry bit precision (prec), LUT size power (lutsp) and how many max bits |b-a| takes (abdp)
	const uint32_t outfmt = 30;	// final output format in s1.outfmt
...
static INLINE int32_t fpgauss_d1i(int32_t x)	// Gaussian function in the [-4 , 4] range, input is 3+.16, output is 1.30
{
...
static INLINE int32_t fpgauss_d0(int32_t x)	// Gaussian function in the [-4 , 4] range, input is 3+.16, output is 1.30
{
...
static INLINE int32_t fperfr_d1i(int32_t x)	// 0.5erf(x)+0.5 function in the [-4 , 4] range, input is s3+.16, output is 1.30
{
...
static INLINE int32_t fperfr_d0(int32_t x)	// 0.5erf(x)+0.5 function in the [-4 , 4] range, input is s3+.16, output is 1.30
{
