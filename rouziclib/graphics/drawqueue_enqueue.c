...
int dqnq_thread(framebuffer_t *fbp)
{
	size_t read_pos;
	int32_t read_pos_next, write_pos;
	enum dqnq_type type;
	uint8_t *dp = (uint8_t *) fbp->dqnq_data;

	fb = *fbp;

start:
	// Read the read and write positions
	rl_mutex_lock(&fbp->dqnq_mutex);
	read_pos = fbp->dqnq_read_pos;
	write_pos = fbp->dqnq_write_pos;
	rl_mutex_unlock(&fbp->dqnq_mutex);

	// Skip end bytes if necessary and return to the start of the buffer
	if (fbp->dqnq_as - read_pos < DQNQ_TYPE_SIZE)
		read_pos = 0;
	...
	if (read_pos == write_pos)
	{
		...
		if (rl_atomic_load_i32(&fbp->dqnq_end_wait_flag) == 1)
		{
			rl_atomic_store_i32(&fbp->dqnq_end_wait_flag, 2);
			goto start;
		}

		// Unblock the main thread if it's waiting
		if (rl_atomic_load_i32(&fbp->dqnq_end_wait_flag) == 2)
		{
			rl_atomic_store_i32(&fbp->dqnq_end_wait_flag, 0);
			rl_sem_post(&fbp->dqnq_end_sem);
		}

		// Wait if main thread isn't waiting at the end
		if (rl_atomic_load_i32(&fbp->dqnq_end_wait_flag) == 0)
		{
			// Flag the main thread to unblock this thread when it adds new data
			rl_atomic_store_i32(&fbp->dqnq_read_wait_flag, 1);

			// Wait for the main thread to unblock this thread
			rl_sem_wait(&fbp->dqnq_read_sem);
		}
	...
	while (fbp->dqnq_thread_on)
	{
		// Read type
		type = dqnq_read_type_id(fbp->dqnq_data, &read_pos);

		// Calculate next entry's position
		read_pos_next = read_pos + dqnqt_arg_size[type];
		if (fbp->dqnq_as - read_pos_next < DQNQ_TYPE_SIZE)
			read_pos_next = 0;
		...
		rl_atomic_store_i32(&fbp->dqnq_read_pos, read_pos_next);
