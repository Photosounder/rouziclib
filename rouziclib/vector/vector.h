...
extern void draw_vobj_fullarg(framebuffer_t fb, vobj_t *o, xy_t p, xy_t scale, double angle, double line_thick, col_t colour);
extern xy_t *vobj_to_map(vobj_t *o, int32_t dofree);
extern vobj_t *map_to_vobj(xy_t *map, int32_t mapsize, int32_t dofree);

#define draw_vobj(fb, o, p, scale, angle, line_thick, colour)	draw_vobj_fullarg(fb, o, p, set_xy(scale), angle, line_thick, colour)
