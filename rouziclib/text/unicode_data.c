...
void unicode_data_set_lut(int16_t **lut, uint32_t cp, int16_t index)
{
	// Add LUT section if needed
	if (lut[cp >> LUT_SHIFT] == NULL)
		lut[cp >> LUT_SHIFT] = calloc((1 << LUT_SHIFT), sizeof(int16_t));
...
void make_unicode_data_table_file()	// generates the unicode_data_table.h file
{
	...
	end = codepoint_lut[0x10FFFD >> LUT_SHIFT][0x10FFFD & LUT_MASK];
	for (i=0; i <= end; i++)
	{
		e = ucd[i];
