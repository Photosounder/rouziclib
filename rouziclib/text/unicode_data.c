...
int32_t **codepoint_lut=NULL;

#define LUT_SHIFT 11
#define LUT_MASK ((1<<LUT_SHIFT)-1)
...
void unicode_data_set_lut(int32_t **lut, uint32_t cp, int32_t index)
{
	// Add LUT section if needed
	if (lut[cp >> LUT_SHIFT] == NULL)
		lut[cp >> LUT_SHIFT] = calloc((1 << LUT_SHIFT), sizeof(int32_t));

	// Add LUT reference
	lut[cp >> LUT_SHIFT][cp & LUT_MASK] = index;
}

unicode_data_t *load_unicode_data_from_file(uint8_t *ucd_path, int32_t ***codepoint_lut)
{
	FILE *udf;
	...
	*codepoint_lut = (int32_t **) calloc(0x110000>>LUT_SHIFT, sizeof(int32_t *));
	...
	while (fgets(line, sizeof(line), udf))
	{
		...
		{
			// Put the table ID into the LUT taking codepoints as index
			sscanf(a, "%x", &ucd[ic].codepoint);
			unicode_data_set_lut(*codepoint_lut, ucd[ic].codepoint, ic);
		}
		...
		if (strstr(ucd[ic].name, "Last"))		// if the name contains Last it means it's the end of a range
		{
			...
			for (i=ucd[ic-1].codepoint+1; i < ucd[ic].codepoint; i++)	// refer all codepoints in the LUT to the first entry
				unicode_data_set_lut(*codepoint_lut, i, ic-1);
		}
...
void init_unicode_data()
{
	...
	{
		// Put the table ID into the LUT taking codepoints as index
		unicode_data_set_lut(codepoint_lut, unicode_data[ic].codepoint, ic);
...
unicode_data_t get_unicode_data(uint32_t c)
{
	...
	if (codepoint_lut[c >> LUT_SHIFT] == NULL)
		return (unicode_data_t) { 0x0000, "", 0, 0, -1, 0, 0, 0, 0 };

	return unicode_data[codepoint_lut[c >> LUT_SHIFT][c & LUT_MASK]];
}

void make_unicode_data_table_file()	// generates the unicode_data_full_table.h file
{
	int i, end;
	FILE *file;
	unicode_data_t *ucd, e;
	int32_t **file_codepoint_lut=NULL;

	// Load the source table without replacing the runtime lookup table
	ucd = load_unicode_data_from_file("../exp/unicode/UnicodeData.txt", &file_codepoint_lut);
	// Stop if the source table cannot be loaded
	if (ucd==NULL)
		return ;

	// Find the last source entry needed by the generated table
	end = file_codepoint_lut[0x10FFFD >> LUT_SHIFT][0x10FFFD & LUT_MASK];
	file = fopen_utf8("../rouziclib/text/unicode_data_full_table.h", "wb");
	// Handle output file failures
	if (file==NULL)
	{
		// Release the temporary source table after an output failure
		for (i=0; i <= end; i++)
			free(ucd[i].name);
		free(ucd);
		free_2d((void **) file_codepoint_lut, 0x110000>>LUT_SHIFT);
		return ;
	}
	...
	// Release the temporary source table after writing the generated file
	for (i=0; i <= end; i++)
		free(ucd[i].name);
	free(ucd);
	free_2d((void **) file_codepoint_lut, 0x110000>>LUT_SHIFT);
